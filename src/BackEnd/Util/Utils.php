...
class Utils
{
    ...
    /** Estados Brasileiros ou União Federativa **/
    public static $StatesOfBrazil = array('AC' => 'Acre', 'AL' => 'Alagoas', 'AM' => 'Amazonas', 'AP' => 'Amapá', 'BA' => 'Bahia', 'CE' => 'Ceará', 'DF' => 'Distrito Federal', 'ES' => 'Espírito Santo', 'GO' => 'Goiás', 'MA' => 'Maranhão', 'MG' => 'Minas Gerais', 'MS' => 'Mato Grosso do Sul', 'MT' => 'Mato Grosso', 'PA' => 'Pará', 'PB' => 'Paraíba', 'PE' => 'Pernambuco', 'PI' => 'Piauí', 'PR' => 'Paraná', 'RJ' => 'Rio de Janeiro', 'RN' => 'Rio Grande do Norte', 'RO' => 'Rondônia', 'RR' => 'Roraima', 'RS' => 'Rio Grande do Sul', 'SC' => 'Santa Catarina', 'SE' => 'Sergipe', 'SP' => 'São Paulo', 'TO' => 'Tocantins',);

    public static function FormatSQLException($message)
    {
        $string = '';
        try
        {
            $string = $message;
            $string = preg_replace('/(SQLSTATE\[[0-9]{1,}\]:)/i', '%%%', $string);
            $string = preg_replace('/(ERRO:)/i', '%%%', $string);
            $string = preg_replace('/(DETAIL:)/i', '%%%', $string);
            $string = preg_replace('/(\' in \/)/i', '%%%', $string);
            $string = preg_split('(%%%)', $string);
            $string = trim($string[1]) . ' ' . trim($string[2]) . ' ' . trim($string[3]);
        }
        catch (Exception $e)
        {
            return $string = '';
