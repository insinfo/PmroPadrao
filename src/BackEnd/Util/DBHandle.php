...
class DBHandle
{
    ...
    public static $ARRAY_NUM_FORMAT = 'arrayNum';

    function __construct()
    {
    }

    public function Connect($connectionName = DBConfig::DEFAULT_CONNECTION,PDO $pdoInstance=null)
    {
    ...
    public function SelectAll($tableName, $fields = null, $conditions = null, $returnType = 'array', $advOptions = null)
    {
        ...
        else if ($returnType == self::$ARRAY_NUM_FORMAT)
        {
    ...
    public function Insert($tableName,ContentValues $contentValues, $returnId = false, $tableIdSeq = null)
    {
        $result = '-1';
            ...
            $query = 'INSERT INTO "' . $tableName . '" (' . $contentValues->getColumns() . ') VALUES (' . $contentValues->getValues($this->connection) . ') ';
            //echo $query.'<br>';
    ...
    public function Update($tableName, $conditions,ContentValues $contentValues)
    {
        if ($this->connection == null)
        {
            throw new Exception('Conecção do banco de dados é nula! Conect ao banco antes de chamar esta função', 400);
        ...
        $query = "UPDATE " . $tableName . ' SET  ' . $contentValues->getUpdateString($this->connection) . ' WHERE ' . $this->implodeConditions($conditions);
    ...
    public function UpdateSimple(ContentValues $contentValues)
    {
        ...
        $query = "UPDATE " . $contentValues->getTableName() . ' SET  ' . $contentValues->getUpdateString($this->connection) . ' WHERE ' . $this->implodeConditions($contentValues->getWhereConditions());
    ...
    {
        return $this->connection->quote(str_replace("'","''",$val));
    }
}
