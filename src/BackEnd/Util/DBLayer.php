...
class DBLayer extends Capsule
{
    ...
    public static function getRelation(&$data, $tableName, $localKey, $foreignKey, $relationName = null, $defaultNull=[])
    {
        $db = self::Connect();
        foreach ($data as &$item)
        {
            $extraData = $db->table($tableName)
                ->where($localKey, '=', $item[$foreignKey])->first();
            if ($relationName)
            {
